    ...
    <div className='min-h-screen bg-gradient-to-br from-gray-900 via-gray-900/95 to-gray-900/90 text-white px-4 sm:px-[5%] md:px-[8%] py-6'>
      <div className='flex flex-col items-center md:flex-row gap-4 mb-6 bg-gray-800/30 backdrop-blur-lg p-4 rounded-xl border border-emerald-500/20'>
        ...
        <div className='text-center md:text-left'>
          ...
          <p className='mt-2 text-sm text-gray-300/80'>Rank: #{coinDetails.market_cap_rank}</p>
        </div>
      </div>

      {/* Chart and Selector */}
      <div className='mb-6 bg-gray-800/30 backdrop-blur-md p-4 rounded-xl border border-emerald-500/20'>
        <div className='flex flex-col md:flex-row justify-between items-start sm:items-center mb-3 gap-2'>
          ...
          <div className='relative group'>
            ...
            <div className='absolute -inset-0.5 bg-gradient-to-r from-emerald-600/20 to-cyan-500/20 rounded-lg blur opacity-30 group-hover:opacity-50 duration-300 -z-10' />
          </div>
        </div>
      ...
      <div className='space-y-3 md:hidden'>
        {[
          ['Current Price', coinDetails.market_data.current_price[currentCurrency.name]],
          ['Market Cap', coinDetails.market_data.market_cap[currentCurrency.name]],
        ].map(([label, value]) => (
          <div key={label} className='bg-gray-800/30 backdrop-blur-md p-4 rounded-lg border border-emerald-500/20'>
            <div className='flex justify-between items-center'>
              <span className='text-sm text-cyan-400/80'>{label}</span>
      ...
      <div className='hidden md:grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6'>
        {[
          ...
          <div key={label} className='bg-gray-800/30 backdrop-blur-md p-4 rounded-xl border border-emerald-500/20'>
            <h3 className='text-sm text-cyan-400/80 mb-2'>{label}</h3>
      ...
      <div className='space-y-3 md:space-y-0 md:grid md:grid-cols-2 lg:grid-cols-2 gap-4'>
        <div className='bg-gray-800/30 backdrop-blur-md p-4 rounded-lg md:rounded-xl border border-emerald-500/20'>
          <div className='flex justify-between items-center'>
            <span className='text-sm md:text-base text-cyan-400/90'>24h Change</span>
